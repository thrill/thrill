...
int page_rank(Context& ctx) {
    ...
    DIARef<PageWithRank> ranks = links.Map([](PageWithLinks input) {
    ...
    for (std::size_t i = 1; i <= 10; ++i) {
        ...
        auto pages = links.Zip([](PageWithLinks first, PageWithRank second) {
                    return std::make_tuple(std::get<0>(first), std::get<1>(second), std::get<1>(first));
                }, ranks);

        auto contribs = pages.FlatMap<PageWithRank>([](Page page, auto emit) {
