...
#pragma once
#ifndef THRILL_EXAPLES_STOCHASTIC_GRADIENT_DESCENT_STOCHASTIC_GRADIENT_DESCENT_HEADER
#define THRILL_EXAPLES_STOCHASTIC_GRADIENT_DESCENT_STOCHASTIC_GRADIENT_DESCENT_HEADER
...
template <typename Vector>
using GradientResult = std::pair<Vector, double>; // weights, loss
template <typename Vector>
using SumResult = std::pair<GradientResult<Vector>, size_t>; // size_t for counting the actual number of data points
...
class StochasticGradientDescent {
    public:
        StochasticGradientDescent(size_t num_iterations, double mini_batch_fraction, double step_size, double tolerance):
            num_iterations(num_iterations), mini_batch_fraction(mini_batch_fraction), step_size(step_size), tolerance(tolerance) {}

        //! do the actual computation
        Vector optimize(const DIA<DataPoint<Vector>> &input_points, Vector& initial_weights) {
            ...
            while (!converged && i <= num_iterations) {
                ...
                auto sum_result = sample
                    .Map([&weights](const DataPoint<Vector> p) {
                            auto grad = LeastSquaresGradient<Vector>::Compute(p.data, p.label, weights);
                    ...
                    .Sum(
                        ...
                        return SumResult<Vector>(
                                // gradient, loss
                                GradientResult<Vector>(grad_a.first + grad_b.first, grad_a.second + grad_b.second),
                                // number of data points (BernoulliSample yields only an approximate fraction)
                                n_a + n_b);
                        },
                        SumResult<Vector>(GradientResult<Vector>(Vector::Make(weights.size()).fill(0.0), 0.0), 0));
