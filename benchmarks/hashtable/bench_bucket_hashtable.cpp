...
int main(int argc, char* argv[]) {
    ...
    std::vector<c7a::data::Emitter<IntPair>> emitter;
    for (size_t i = 0; i < workers; i++) {
        emitter.emplace_back(manager.GetLocalEmitter<IntPair>(manager.AllocateDIA()));
    }

    c7a::core::ReducePreTable<decltype(key_ex), decltype(red_fn), c7a::data::Emitter<IntPair>>
    table(workers, num_buckets_init_scale, num_buckets_resize_scale, max_num_items_per_bucket,
          max_num_items_table, key_ex, red_fn, emitter);
