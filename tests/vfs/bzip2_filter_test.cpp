/*******************************************************************************
 * tests/vfs/bzip2_filter_test.cpp
 *
...
TEST(BZip2FilterTest, WriteReadSingleFile) {
    vfs::TemporaryDirectory tmpdir;

    {
        vfs::WriteStreamPtr ws = vfs::SysOpenWriteStream(
            tmpdir.get() + "/test.dat.bz2");

        vfs::WriteStreamPtr zs = vfs::MakeBZip2WriteFilter(ws);
    ...
    }
    {
        ...
        vfs::ReadStreamPtr zs = vfs::MakeBZip2ReadFilter(rs);
